...
document.head.appendChild(style);
  ...
  renderFallbackFontList(fonts) {
    const container = $("fm-registeredFontList");
    if (!container) return;
    container.innerHTML = "";
    const list = document.createElement("div");
    list.className = "fm-fallbackFontList";
    fonts.forEach((font) => {
      const item = document.createElement("button");
      item.type = "button";
      item.className = "fm-fallbackFontItem";
      item.textContent = font.name;
      item.style.cssText = "display:block;width:100%;margin:4px 0;padding:6px 8px;text-align:left;background:#2f2f2f;color:#f5f5f5;border:1px solid #555;border-radius:4px;cursor:pointer;";
      item.onclick = async () => {
        await fontManager.unregisterFont({ value: font.name });
    ...
    if (!fonts.length) {
      ...
      empty.style.cssText = "color:#aaa;font-size:12px;padding:8px 0;";
      empty.textContent = getText("ft_registered") || "Registered fonts";
  ...
  async updateFontList() {
    ...
    if (!this.tagify) {
      this.renderFallbackFontList(fonts);
      return;
    }
    // console.log(
