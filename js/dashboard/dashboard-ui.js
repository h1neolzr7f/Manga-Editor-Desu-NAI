...
var dashboardLibraryLoadPromise=null;
var MODE_LABELS={
T2I:'T2I',
I2I:'I2I',
};
...
var existing=document.querySelector('script[data-dashboard-lib="'+src+'"]');
if(existing){
if(existing.dataset.dashboardLoaded==='true'||existing.readyState==='complete'){
resolve();
return;
}
...
script.async=true;
script.defer=true;
script.dataset.dashboardLib=src;
script.onload=function(){
script.dataset.dashboardLoaded='true';
resolve();
};
...
function ensureDashboardLibraries(){
if(!dashboardLibraryLoadPromise){
dashboardLibraryLoadPromise=Promise.all([
loadScriptOnce('cdn-local/chart.js','Chart'),
loadScriptOnce('cdn-local/wordcloud.js','WordCloud'),
]).catch(function(error){
dashboardLibraryLoadPromise=null;
dashboardLogger.error('Failed to load dashboard libraries',error);
throw error;
});
}
return dashboardLibraryLoadPromise;
}
function open(){
if(isOpen) return;
isOpen=true;
...
async function init(){
await ensureDashboardLibraries();
if(isInitialized){
await refresh();
return;
}
