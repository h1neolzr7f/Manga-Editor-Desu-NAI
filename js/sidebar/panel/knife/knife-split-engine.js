 ...
 */
/**
 * 按固定比例直线切分（无随机偏移，用于分镜模板）
 * @param {Object} panel
 * @param {boolean} isVertical true=竖线（左右分）
 * @param {number} ratio 0~1，分割线位置
 * @returns {{isSplit:boolean,polygon1?:Object,polygon2?:Object}}
 */
function guidedSplitPanel(panel,isVertical,ratio) {
if (!panel) return {isSplit: false};
var bounds=panel.getBoundingRect?panel.getBoundingRect(true):null;
if (!bounds) return {isSplit: false};
ratio=Number(ratio);
...
var angle;
if (isVertical) {
centerX=bounds.left+bounds.width*ratio;
centerY=getCenterYByFabricObject(panel);
angle=(90)*(Math.PI/180);
} else {
centerX=getCenterXByFabricObject(panel);
centerY=bounds.top+bounds.height*ratio;
angle=0;
}

var pointAtpx=getPointAtDistance(centerX,centerY,angle,KNIFE_CONSTANTS.SPLIT_LINE.POINT_DISTANCE);
var splitLine=drawLine(centerX,centerY,pointAtpx.x,pointAtpx.y,panel);
if (splitLine===null) return {isSplit: false};

var splitResult=splitPolygon(panel);
if (!splitResult.isSplit||!splitResult.polygon1||!splitResult.polygon2) {
return {isSplit: false};
}

var area1AspectRatio=splitResult.polygon1.width/splitResult.polygon1.height;
var area2AspectRatio=splitResult.polygon2.width/splitResult.polygon2.height;
if (area1AspectRatio<=KNIFE_CONSTANTS.ASPECT_RATIO.MIN||
area1AspectRatio>=KNIFE_CONSTANTS.ASPECT_RATIO.MAX||
area2AspectRatio<=KNIFE_CONSTANTS.ASPECT_RATIO.MIN||
area2AspectRatio>=KNIFE_CONSTANTS.ASPECT_RATIO.MAX) {
canvas.add(setNotSave(panel));
canvas.remove(setNotSave(splitResult.polygon1));
canvas.remove(setNotSave(splitResult.polygon2));
return {isSplit: false};
}
return splitResult;
}
