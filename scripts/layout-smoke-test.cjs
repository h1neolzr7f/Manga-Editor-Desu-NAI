...
must(html.includes('id="canvasEmptyHintDismiss"'), 'empty hint dismiss button missing');
must(html.includes('data-action="dismissEmptyCanvasHint"'), 'empty hint close action missing');
must(html.includes('data-action="dismissEmptyCanvasHintCrop"'), 'empty hint crop-dismiss action missing');
must(/id="canvasEmptyHint"[^>]*hidden/.test(html), 'empty hint must start hidden');
must(html.includes('自己裁剪'), 'empty hint custom-crop path missing');
