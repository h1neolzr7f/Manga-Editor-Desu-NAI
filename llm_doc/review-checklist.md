...
| 29 | 空画布 | 空画布提示必须可关闭；自定义页面/自己切格子时不得挡住画布；模拟器与选分镜引导应放在教程里。测试不得只断言提示存在，必须覆盖「自定义页面 + 切割格子」 | 2 |
| 30 | 分镜 | 自定义页面必须与竖页/横页一样铺满整页 `isPanel` 格子，否则刀切没有目标 | 1 |
| 31 | 文案 | 出图 preflight / 随机切 / HUD 不得只写「去选模板」；必须同时写「页面 → 自定义页面 / 切割格子」 | 1 |
| 32 | 空画布 | `loadBookSize(..., addPanel=false)` 会在启动时调用，禁止在该函数里持久关闭空画布提示，只能由用户点页面管理按钮关闭 | 1 |
